...
const initPayment = async (paymentData: any) => {
  try {
    ...
    const response = await axios({
      method: "post",
      url: config.ssl.sslPaymentApi as string,
      data: data,
    ...
    throw new ApiError(status.BAD_REQUEST, "Payment Error");
  }
};

const validatePaymet = async (payLoad: any) => {
  try {
    const response = await axios({
      method: "GET",
    ...
    throw new ApiError(status.BAD_REQUEST, "Invalide Payment");
  }
};
